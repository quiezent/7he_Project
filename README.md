...
- For a weather- or haze-sensitive Bukit Kiara decision, run `python tools/ride_conditions.py`. The sole evidence URL remains `http://192.168.80.147:8765/api/v1/mtb/environment-evidence`; the consumer contract is now schema `1.6.0`, last verified at revision `sha256:ba6c38068f20213c`.
- The response advertises OpenAPI at `/api/openapi.json`, JSON Schema at `/api/v1/mtb/environment-evidence/schema`, and the living guide at `/docs/coach-api.md`. The adapter suppresses repeat passive fetches inside the advertised 180-second cadence. On a `schemaVersion`/`contract.revision` change it retains accepted last-known-good evidence and raises `revalidation_needed`; it does not silently fetch or cache contract resources. The engineering owner must resolve those paths against the evidence origin, revalidate them with `If-None-Match` where a cached copy exists, review the contract, and then update the verified schema/revision in canonical config. The revision describes the contract and documentation, not live evidence freshness; never call the dashboard's internal refresh route.
...
- The decision surface uses raw PM2.5 sport-exercise bands directly: below 25 ug/m3 normal, 25 to below 51 moderate caution, 51-150 poor exercise conditions, and above 150 likely hazardous outdoors. These are not AQI values. The current horizon is exactly 90 minutes to arrival plus a modeled 120-minute ride, covering offsets +90 through +210 minutes. Morning and afternoon compare equal 120-minute sessions, not arbitrary all-day conditions.
- Arrival/on-trail analogue points and sensor-anchored CAMS window points are explicitly aggressive and experimentally unvalidated. Use their separate persistence anchors, conservative envelopes, upper-risk markers, support, calibration and limitations. A relative `preferredWindow` can guide planning or caution only: `rideApproval=false`. Use a window only for its exact target local date and modeled interval, and perform the stated fresh recheck before commitment.
- Ordinary rain is trail/logistics context only and never chooses the preferred window. Structured thunderstorm evidence is a safety comparison input when `usedForDecision=true`, fresh nearby-storm evidence can affect only a near-term window, and apparent heat is a tie-breaker. None is direct ride approval.
- Automatic environment application remains limited to matching Bukit Kiara/TTDI outdoor candidates and may retain a restriction, hold for recheck, downshift consequence or replace that candidate when its ceiling is lower. Athlete-reported Denai Peladang is about 18 km from Kiara, so Kiara evidence is at most regional particle-precaution context there; it cannot establish DP weather, thunderstorm risk, trail state, direct PM2.5 clearance or ride approval. DP automation requires a venue-specific source. The environment layer cannot promote readiness, establish indoor air quality, or automatically increase duration, intensity or technical consequence. There is no legacy local-endpoint or IQAir fallback; an unavailable/expired source leaves the environment unknown.
...
- `snapshots/environment_evidence.json` / `snapshots/environment_evidence_YYYY-MM-DD.json`
  - bounded normalized current/daily evidence from the athlete-managed schema-1.6 Bukit Kiara endpoint, with latest-attempt and last-known-good state, raw PM2.5/PM10, exact-horizon experimental exposure uncertainty, weather/thunderstorm context and limitations; never the raw response or server history, and usable only as a Bukit Kiara/TTDI hold/downshift ceiling
